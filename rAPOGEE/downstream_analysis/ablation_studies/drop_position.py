...
# Define predefined positional features to exclude from the main feature set
position_features = ['x_coord', 'y_coord', 'z_coord', 'closest_modification', 'closest_modification_2D']
...
X = pd.read_csv("../../checkpoints/mithril_features.csv", sep="\t", index_col=0).drop(position_features, axis=1)
...
cross_validation_result = cross_validate(
    model_rf, X_extended, y, scoring=binary_auROC,
...
pk.dump((cv_ground_truth, cv_predictions), open("./ablation_cv/cv_predictions_no_position.pk", "wb"))