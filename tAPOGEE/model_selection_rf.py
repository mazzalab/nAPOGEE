...
gridsearch_rf = GridSearchOOB_BinaryClassifier(
    estimator=BalancedRandomForestClassifier(
        n_estimators=500, oob_score=True, replacement=False,
        sampling_strategy={0:75, 1:75}, bootstrap=True,
        n_jobs=-1
    ),
...
tuned_model_rf = GridSearchCV(
    estimator=model_rf, param_grid=preprocess_grid,
...
tuned_model_rf.fit(X_extended, y)
pk.dump(tuned_model_rf, open("checkpoints/candidate_models/model_rf.pk", "wb"))

print("################## END ##################")
print("OOB gridsearch params:")
print(tuned_model_rf.best_estimator_[-1].best_params, tuned_model_rf.best_estimator_[-1].best_score)
print("CV gridsearch params:")
print(tuned_model_rf.best_params_, tuned_model_rf.best_score_)